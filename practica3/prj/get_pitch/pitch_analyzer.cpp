...
namespace upc {
  void PitchAnalyzer::autocorrelation(const vector<float> &x, vector<float> &r) const {
    ...
    for (i=0; i< size; ++i) {
      sum = 0;
      for(j=0; j<size-i; j++){ //for each calculate sum
        sum += x[j]*x[j+i];
      }
      r[i] = sum; 
    }
  ...
  void PitchAnalyzer::set_window(Window win_type) {
    ...
    //It can return a rectangular or Hamming window. In both cases it will have FrameLen lenght
    if (frameLen == 0)
      return;
  ...
  float PitchAnalyzer::compute_pitch(vector<float> & x) const {
    ...
    //POST PROCESADO - FILTO DE MEDIANA
    //TODO: IMPLEMENTAR EN EL LAB
    ...
    for (int i=0; i<frameLen; i++){
      ...
      if (firstNegative){
        ...
        if (r[i]>maxVal)  index = i;

      }else if(r[i]<0){ //En cas que sigui menor que zero  
        firstNegative = true;
      }
    }
    
    float frequency = 1/index*samplingFreq;
